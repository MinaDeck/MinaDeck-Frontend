...
export default function PK() {
  const {
    data: gamePlayerInfo,
    mutate: gamePlayerInfoMutate,
  } = useSWR('local:gamePlayerInfo', stateFetcher);
      ...
      <div className='transition-transform duration-300 delay-200 -translate-x-full group-[.opacity-100]:translate-x-0 absolute top-0 left-0 w-full h-full bg-no-repeat bg-[url("/pk.png")] flex items-center justify-start'>
        <div className='flex relative left-48 top-2'>
          <div className='relative top-4 left-10'>
            <Avatar
              hiascent={user0?.userId !== gamePK?.WinUserId}
              user={user0}
              userId={gamePlayerInfo?.userId}
              headPic={user0?.headPic}
              dealer={user0?.isBanker}
            />
            <div className='absolute -left-16 bottom-8 w-32 text-center font-extrabold text-white text-sm [text-shadow:1px_1px_0_rgb(0,0,0),-1px_-1px_0_rgb(0,0,0),1px_-1px_0_rgb(0,0,0),-1px_1px_0_rgb(0,0,0)]'>
              {displayAddress(gamePlayerInfo?.address)}
            </div>
          </div>
      ...
      <div
        ...
        <div className='flex relative right-60 top-4'>
          ...
          <div className='relative top-6 left-36'>
            <Avatar
              hiascent={user1?.userId !== gamePK?.WinUserId}
              user={user1}
              userId={user1?.userId}
              headPic={user1?.headPic}
              dealer={user1?.isBanker}
            />
            <div className='absolute top-2/3 right-1/3 w-32 bottom-3 text-center font-extrabold text-white text-sm [text-shadow:1px_1px_0_rgb(0,0,0),-1px_-1px_0_rgb(0,0,0),1px_-1px_0_rgb(0,0,0),-1px_1px_0_rgb(0,0,0)]'>
              {displayAddress(gamePlayerInfo?.address)}
            </div>
          </div>
        </div>
