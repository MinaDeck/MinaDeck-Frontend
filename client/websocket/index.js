...
wss.on('connection', function connection(ws, req) {
  ...
  ws.on('message', function incoming(message) {
    ...
    if (newData.state == undefined) {
      ...
      // Check if gameData already contains an object with a 'game' key
      const existingGameData = gameData.find(obj => obj.hasOwnProperty('game'));
      if (existingGameData) {
        // Update the existing game object with the new data
        Object.assign(existingGameData.game, newData.game);
      } else if (newData.game) {
        // Insert the new game object into gameData
        gameData.push({ game: newData.game });
      }
